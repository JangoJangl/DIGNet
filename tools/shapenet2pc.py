...
NUM_POINTS = 8192
shapenet_meshes_path = '/home/i53/student/jandl/repos/DIGNet/dataset/acronym/meshes' # https://shapenet.cs.stanford.edu/shapenet/obj-zip/ShapeNetCore.v2.zip
out_folder = 'pointclouds_' + str(NUM_POINTS)
import os,sys
import numpy as np
import trimesh
...
def process_point_cloudv2(path):
    ...
    out_dir = path.replace('meshes', out_folder)
    ...
    for f in files:
        ...
        out_path = f.replace('meshes', out_folder).replace('.obj', '.npy')
...
if __name__ == '__main__':
    paths = glob.glob(shapenet_meshes_path + '*/*')
    print('detected obj files=', len(paths))
    num_cats = len(paths)
    a = os.path.exists(shapenet_meshes_path.replace('meshes', out_folder))
    if not os.path.exists(shapenet_meshes_path.replace('meshes', out_folder)):
        os.mkdir(shapenet_meshes_path.replace('meshes', out_folder))
